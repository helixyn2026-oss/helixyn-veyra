...
import CEONavLinks from "@/app/components/CEONavLinks";

export const metadata: Metadata = {
  title: "Executive Portal | Helixyn ELMS",
  description: "Executive workspace for Helixyn",
};
...
export default async function CEOLayout({
  ...
  const userName = user?.name || "Chief Executive";
  const userEmail = user?.email || "ceo@helixyn.com";
  ...
  return (
    <div className="flex h-screen bg-slate-950 font-sans text-slate-300 overflow-hidden">
      ...
      <aside className="w-[260px] bg-slate-900 border-r border-slate-800 flex flex-col h-full shrink-0 transition-all duration-300 relative z-20">
        ...
        <div className="h-16 flex items-center px-6 border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-3 w-full">
            <div className="w-8 h-8 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg flex items-center justify-center shrink-0 text-white shadow-lg shadow-emerald-500/20">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Executive</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1.5 custom-scrollbar">
          <div className="mb-4 px-2 relative">
             ...
             <input type="text" placeholder="Search..." className="w-full bg-slate-950 border border-slate-800 rounded-md py-1.5 pl-9 pr-3 text-xs focus:outline-none focus:border-emerald-500/50 text-white placeholder-slate-600" />
          </div>
        ...
        <div className="p-4 border-t border-slate-800 shrink-0 bg-slate-900/50">
          <Link href="/login" className="flex items-center gap-3 w-full text-left p-2 rounded-lg hover:bg-slate-800 transition-colors border border-transparent hover:border-slate-700">
            ...
            <LogOut size={16} className="text-slate-500 hover:text-emerald-500 transition-colors" />
          </Link>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-950 relative">
        ...
        <div className="absolute top-0 left-1/4 w-[50%] h-[300px] bg-emerald-500/5 blur-[150px] rounded-full pointer-events-none"></div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-8 shrink-0 z-10 sticky top-0">
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-bold text-white">Executive Command</h2>
            <span className="bg-slate-800 border border-slate-700 text-emerald-400 p-1 rounded text-xs">
              <Crown size={14} />
            </span>
          </div>
